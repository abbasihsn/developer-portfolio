...
const ProjectView = () => {
    ...
    let widget = <div className={classes.project_view__container}>
        {projects.map(item => <ProjectItem onClick={onClickHandler} title={item.title} description={item.description} src={item.src} id={item.id} tools={item.tools} type={item.type} key={item.id} demo={item.demoLink} code={item.codeLink}/>)}
    </div>
